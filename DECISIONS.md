...
## 2026-09-14 — Lot 13 : les données restent sur l'appareil

- **2026-09-14** — **Demande du commanditaire : plus de Supabase, tout en interne.** Deux lectures possibles — écrire les données dans le navigateur, ou héberger une base avec le site. Retenu : **le navigateur, derrière une couche d'accès unique**, pour garder la porte ouverte. Déploiement sans configuration, sans compte, sans coût, et hors-ligne par construction plutôt que par rattrapage.
- **2026-09-14** — La bascule tient parce qu'il y a **une seule porte** : `lib/donnees/depot.ts`. L'interface `Depot` a été écrite avant l'implémentation, en recopiant les opérations que faisaient les Server Actions. Aucun composant ne connaît IndexedDB. Rebrancher une base, c'est un second dépôt et une ligne dans `depot()`.
- **2026-09-14** — Les **agrégats sont dans le dépôt**, pas dans les composants : `volumeParGroupe`, `tonnageHebdomadaire`, `progressionExercice`, `dernierePerf`. Ce sont les requêtes SQL de demain ; les laisser dans les écrans aurait rendu le rebranchement impossible à tenir.
- **2026-09-14** — **Une séance est un seul document**, qui porte ses exercices et ses séries, là où le schéma relationnel a trois tables. IndexedDB n'a pas de jointure : imiter le relationnel aurait produit trois lectures et un assemblage à la main à chaque écran. La conversion vers les trois tables reste triviale, elle est linéaire.
- **2026-09-14** — Ce que faisaient les **déclencheurs Postgres est repris à l'écriture** : `enregistrerSeance` recalcule le tonnage (ex-`recalculer_volume_seance`) et met à jour les records (ex-`series_enregistrer_records`). `supprimerSeance` **rejoue toutes les séances dans l'ordre** pour reconstruire les records, parce qu'un record supprimé ne peut pas être deviné.
- **2026-09-14** — Le 1RM d'Epley n'a plus de source de vérité SQL : `lib/calculs.ts` **devient** la source. Le test de parité avec la fonction Postgres est conservé — c'est lui qui garantit que le chemin de retour donnera exactement les mêmes chiffres.
- **2026-09-14** — Les 131 exercices sont générés en TypeScript par `scripts/generer-exercices-ts.py`, qui **importe la même liste Python** que le seed SQL. Deux sorties, une source : le module et le seed ne peuvent pas diverger.
- **2026-09-14** — `supabase/` est **conservé en entier** — 9 migrations, le seed, les gabarits d'e-mail, 75 assertions RLS — et `npm run db:verif` continue de le rejouer. Un chemin de retour qu'on ne teste plus n'est pas un chemin de retour, c'est un souvenir.
- **2026-09-14** — **Ce qui disparaît, et pourquoi c'est assumé** : le classement de la salle, l'espace admin, les codes d'accès, le partage de programme par code, les relances push. Tous supposent que deux téléphones se parlent. Les garder en façade aurait été mentir sur ce que fait l'app.
- **2026-09-14** — Les **relances push sont remplacées par un bandeau d'encre** sur le tableau de bord (« Trois jours sans séance »). Une notification exige un serveur ; le fait, lui, est lisible localement. La règle de ton ne change pas : un constat, pas un reproche.
- **2026-09-14** — Les **photos de progression sont stockées en `Blob`** dans IndexedDB, pas en base64 : une photo de téléphone gonfle d'un tiers en base64 et le quota du navigateur est la ressource rare. Les URL d'objet sont créées à l'affichage et **révoquées au démontage**.
- **2026-09-14** — La sauvegarde JSON **exclut les photos**, volontairement : un carnet d'un an tient en quelques dizaines de kilo-octets, les photos le feraient passer à plusieurs dizaines de méga-octets et le téléchargement échouerait sur mobile.
- **2026-09-14** — Le profil expose le **quota de stockage** et un bouton « Demander la conservation » (`navigator.storage.persist()`). Sans cela, un navigateur peut effacer IndexedDB sous pression disque — c'est le seul vrai risque du choix local, il doit être visible et non enfoui.
- **2026-09-14** — L'écriture de la séance en cours est **débattue à 500 ms** et vidée sur `visibilitychange` : écrire à chaque frappe sur un incrémenteur ferait des dizaines d'écritures par série. L'indicateur garde trois états, dont `refuse` — en navigation privée, le stockage échoue et il faut le dire.
- **2026-09-14** — La **CSP se resserre** : `connect-src 'self'`, `img-src 'self' data: blob:`. Plus rien ne sort. C'est la contrepartie agréable de la bascule — la surface d'attaque réseau est nulle.
- **2026-09-14** — `.env.local.example` est **vidé de ses clés** et ne garde qu'une note. Un fichier d'exemple qui décrit des variables que le code ne lit plus est pire qu'absent.
- **2026-09-14** — Le parcours Playwright complet **ne s'ignore plus jamais** : il n'a besoin d'aucune base, d'aucun compte, d'aucune confirmation d'e-mail. Il couvre l'onboarding, trois séries validées, un record, le bilan, l'historique, et la survie à un rechargement de page. C'était l'argument le plus concret du changement.
- **2026-09-14** — Le tableau de qualité du README donne désormais la **plus basse** valeur de performance observée sur trois passes, pas la meilleure : sur une machine partagée l'écart atteint six points, et publier le pic revient à mesurer la chance.
- **2026-09-14** — **Défaut trouvé par ce parcours, pas par relecture** : les molettes d'onboarding *affichaient* 175 cm et 75 kg sans que la valeur soit dans l'état. « Continuer » restait désactivé tant qu'on ne touchait pas la molette. Corrigé en semant les valeurs par défaut au montage.
